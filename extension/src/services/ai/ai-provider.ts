...
// ---- Pro Proxy Provider ----
// Routes AI calls through the backend for Pro users without their own API key

import { aiProxy } from '../api-client';

class ProxyAIProvider implements AIProviderClient {
  readonly provider: AIProvider = 'openai';
  readonly model: string = 'proxy';

  async testConnection(): Promise<boolean> {
    try {
      const result = await aiProxy('Reply with OK', { maxTokens: 5 });
  ...
  async complete(prompt: string, options?: CompletionOptions): Promise<string> {
    ...
    const result = await aiProxy(fullPrompt, {
    ...
    log.debug(`Proxy completion received (${result.response.length} chars, ${result.usage.remaining} remaining)`);
    return result.response;
  }
  ...
  async completeJSON<T = any>(prompt: string, options?: CompletionOptions): Promise<T> {
    ...
    try {
      ...
      throw new Error(`Failed to parse JSON from proxy response: ${raw.substring(0, 200)}`);
    }
  }
}

/**
 * Create a proxy AI provider for Pro-tier users.
 * Routes all AI calls through the backend.
 */
export function createProxyProvider(): AIProviderClient {
  log.info('AI proxy provider created (Pro tier)');
